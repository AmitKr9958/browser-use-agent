...
class _EventDispatchResult(Protocol):
    """Minimal typed boundary for Browser Use's dynamic event-bus result."""

    def event_result(
        self, *, raise_if_any: bool, raise_if_none: bool
    ) -> Awaitable[Any]: ...
    ...
    async def select_tab(self, selector: TabSelector, timeout: float = 10.0) -> TabRecord:
        ...
        if current_url != selected.url or current_title != selected.title:
            ...
            if callable(dispatch):
                try:
                    event = cast(
                        _EventDispatchResult,
                        dispatch(SwitchTabEvent(target_id=selected.target_id)),
                    )
                    await asyncio.wait_for(await_if_needed(event), timeout=timeout)
                    ...
                    logger.error("Tab switch event timed out after %.2fs", timeout)
                    raise TimeoutError(f"Tab switch timed out within {timeout} seconds") from exc
                except TabVerificationError:
                    raise
                except Exception as exc:
                    logger.error("Tab switch via event bus failed: %s", exc, exc_info=True)
                    raise RuntimeError("Failed to switch tab via event bus") from exc
            else:
