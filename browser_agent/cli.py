...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.command == "open":
            result = await open_url(args.url, session)
            print(json.dumps({"success": True, **result}, indent=2))
            return 0
        if args.command == "click":
            result = await click_selector(args.selector, browser_session=session)
            print(json.dumps({"success": True, **result}, indent=2))
            return 0
        ...
        if args.command == "list":
            ...
            print(json.dumps([asdict(tab) for tab in tabs], indent=2))
            return 0
        ...
        if session is not None:
            try:
                ...
                logger.warning("Error stopping CLI browser session: %s", exc, exc_info=True)
...
def main() -> None:
    ...
    except (RuntimeError, LookupError, ValueError, TimeoutError) as exc:
